...
Uses existing rollout data in multicot/mgsm/ if present,
or skips analysis tests gracefully if no data found.

Run:
    python -m multicot.smoke_test
"""
...
# analyze_rollouts.py calls parser.parse_args() at module level.
...
def test_imports():
    ...
    from multicot.prompts import build_base_solution_prompt, build_rollout_prompt, build_dag_labeling_prompt
    from multicot.plots import load_language_chunks
    return True
...
def test_extract_fr_answer():
    from multicot.data_loaders import Problem
    from multicot.utils import extract_answer
...
def test_build_base_solution_prompt_mgsm():
    from multicot.data_loaders import Problem
    from multicot.prompts import build_base_solution_prompt, THINK_ANCHORS
...
def test_build_rollout_prompt_forced():
    from multicot.data_loaders import Problem
    from multicot.prompts import build_rollout_prompt
...
def test_mgsm_loader_en():
    from multicot.data_loaders import MGSMLoader
    loader = MGSMLoader()
...
def test_load_dataset_problems_mgsm_en():
    from multicot.data_loaders import load_dataset_problems
...
def _find_existing_rollout_dir():
    ...
    problem that has chunks.json. The directory structure is:
      multicot/{dataset}/{model_org}/{model_name}/temperature_T_top_p_P/{lang}/correct_base_solution/
    """
    base = Path("multicot")
...
def test_analyze_existing_data():
    ...
    from multicot.analyze_rollouts import analyze_problem
...
def test_plots_generate():
    ...
    if not has_metrics:
        return None  # SKIP (need to run analyze_rollouts first)

    from multicot.plots import (
...
def test_align_chunks_imports():
    ...
    from multicot.align_chunks import (
        embed_chunks,
...
def test_monotone_alignment_dp():
    ...
    from multicot.align_chunks import monotone_max_weight_alignment
    import numpy as np
...
def test_similarity_matrix():
    ...
    from multicot.align_chunks import compute_similarity_matrix
    import numpy as np
    n = 4
...
def test_align_problem_on_existing_data():
    ...
    from multicot.align_chunks import align_problem
    data = align_problem(
...
def test_plot_alignment_scatter():
    ...
    import tempfile
    from pathlib import Path
    from multicot.plots import plot_alignment_importance_scatter
