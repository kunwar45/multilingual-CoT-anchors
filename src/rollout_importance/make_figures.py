...
Generates cross-language comparison plots from compute_importance output.
Loads chunks_labeled.json from problem directories and aggregates metrics.

Usage:
    python -m src.rollout_importance.make_figures --dataset mgsm --languages en,fr,zh --model Llama-3.2-3B-Instruct-Turbo
"""
...
def plot_importance_by_function_tag(
    ...
    if not has_labels:
        print(f"  [skip] No function_tags found — run compute_importance.py first (GPT-4o labeling)")
        return
...
def main():
    ...
    parser.add_argument("--rollouts_base", type=str, default="output/rollouts",
                        help="Base rollouts directory (default: output/rollouts)")
    ...
    if not rollouts_dir.exists():
        ...
        print("Run generate_rollouts.py and compute_importance.py first.")
        return
    ...
    if total == 0:
        print("\nNo labeled chunks found. Run compute_importance.py first to compute importance metrics.")
        return
